...
| Field | Meaning |
|---|---|
| `slideshow.images[]` | Any number of `{src, variant?, loop?, autoAdvanceMs?, syncProgressWithVideo?, title?, subtitle?, body?}` objects. `src` can be either an **image** (`.jpg`/`.png`/`.svg`/etc.) or a **video** (`.mp4`/`.webm`/`.ogg`/`.m4v`/`.mov`) — the renderer auto-detects by file extension. Videos are muted, play from frame 0 the moment the slide becomes current, and pause as soon as it leaves (so nothing plays in the background). `loop` (videos only, default `true`) — set `false` to play once and stop on the last frame. `variant` selects one of the four layouts above (defaults to `default`). `title` is the leading bold portion of the headline (100% white); `subtitle` is rendered inline at 80% white as a continuation. `body` is the paragraph below. All text fields are ignored on the `fullscreen` variant. |
| `slideshow.images[].autoAdvanceMs` | _Optional, per slide._ Overrides `slideshow.autoAdvanceMs` for this slide only. Useful when one slide should linger longer than the rest (e.g. a busy diagram or a longer video clip). Set to `0` to make the slide stay until the viewer navigates manually. Ignored when `syncProgressWithVideo: true` is set on the same slide. |
| `slideshow.images[].syncProgressWithVideo` | _Optional, videos only._ When `true`, the countdown ring tracks the video's playback (`currentTime / duration`) instead of running an independent timer, and the slide advances when the video ends. The pause button pauses / resumes the video together with the ring. Forces `loop: false` (the slide advances on the video's `ended` event, which never fires while looping) and ignores `autoAdvanceMs`. Default `false`. |
