...
# the ESS content version as the kiosk that auto-starts on every login.
#
# Equivalent to running `./kiosk/install.sh app1-ess` in Terminal.
#
...
cat <<'BANNER'
╔══════════════════════════════════════════════════════════════╗
║  Install App 1 — ESS content version (slideshow)             ║
╚══════════════════════════════════════════════════════════════╝

Installing the LaunchAgent that will auto-start App 1 (ESS slideshow)
every time this Mac logs in. If any other App 1 version is currently
installed (OL or Microgrid), it will be uninstalled first — only one
App 1 version may run at a time.

BANNER

./kiosk/install.sh app1-ess
RC=$?

echo
if [[ $RC -eq 0 ]]; then
  cat <<'OK'
✓ App 1 (ESS) is installed.
  Reboot the Mac (or log out + back in) to verify the kiosk starts
  automatically. To start it right now without rebooting:
      launchctl start com.intersolar.app1-ess
OK
else
